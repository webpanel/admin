...
export var EntityForm = function (props) {
    ...
    var entity = props.entity, resourceID = props.resourceID, form = props.form, initialValues = props.initialValues, fields = props.fields;
    ...
    return (React.createElement(ResourceLayer
    // key={version}
    , { 
        // key={version}
        name: entity.name, id: resourceID, fields: entityFields
            .filter(function (f) { return f && f.fetchField() && f.writeable; })
            .map(function (f) { return f.editFetchField(); })
            .filter(function (f) { return f; }), dataSource: entity.dataSource, 
        // onCreate={onCreate}
        initialValues: initialValues, render: function (resource) { return (React.createElement(ResourceForm, __assign({ formResource: resource, onSuccess: function (context) { return handleFormSuccess(resource); } }, form, { render: function (formContext) {
                ...
                return (React.createElement(Spin, { spinning: resource.loading && !resource.polling }, content));
            } }))); } }));
};
