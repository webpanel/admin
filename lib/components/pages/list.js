...
export var EntityList = function (props) {
    ...
    var getListFields = function () {
        ...
        if (_fields) {
            for (var _i = 0, _fields_1 = _fields; _i < _fields_1.length; _i++) {
                ...
                if (!field) {
                    throw new Error("Field '".concat(fieldName, "' not found in entity '").concat(entity.name, "'"));
                }
                listFields.push({
                    field: field,
                    hidden: hidden_1,
    ...
    var cardContent = function (resource, t) {
        ...
        var _card = resolveOptionalThunk(card, resource);
        ...
        return (React.createElement(Card, __assign({ bodyStyle: { padding: "0" }, title: title ||
                t("".concat(entity.name, "._title"), { count: 100, defaultValue: entity.title }), key: "".concat(entity.name, ".table-card") }, _card, { extra: React.createElement(Space, { key: "default-buttons" },
                _card && _card.extra,
