...
export var EntityList = function (props) {
    var getColumns = function (listFields, resource, t) {
        ...
        return listFields.map(function (column) {
            var field = column.field, render = column.render, align = column.align, titleRender = column.titleRender, aggregation = column.aggregation;
            ...
            if (aggregation) {
                col.children = [__assign(__assign({}, col), { title: "aaa" })];
            }
            return col;
        });
    };
    ...
    var cardContent = function (resource, t) {
        ...
        return (React.createElement(Card, __assign({ bodyStyle: { padding: "0" }, title: title ||
                t(entity.name + "._title", { count: 100, defaultValue: entity.title }), key: entity.name + ".table-card" }, card, { extra: [
                card && card.extra,
                React.createElement(Space, { key: "default-buttons" },
            ...
            ].filter(function (x) { return x; }) }), tableContent(resource, t)));
    };
    var tableActionButtons = function (resourceValues, buttons) {
        var entity = props.entity, table = props.table;
    ...
    var tableContent = function (resource, t) {
        ...
        return (React.createElement(React.Fragment, null,
            React.createElement(ResourceTable, __assign({ key: "table_" + resource.name, className: "entitytable", scroll: { x: true }, resourceCollection: resource, pagination: __assign(__assign({}, defaultPagination), (table && table.pagination)), customDetailURL: function (resourceID) {
                    ...
                    return tableActionButtons(values, resolveOptionalThunk(table === null || table === void 0 ? void 0 : table.actionButtons, values));
                }, columns: getColumns(getListFields().filter(function (x) { return !x.hidden; }), resource, t) }))));
    };
    var entity = props.entity, initialFilters = props.initialFilters, initialSorting = props.initialSorting, wrapperType = props.wrapperType, restProps = __rest(props, ["entity", "initialFilters", "initialSorting", "wrapperType"]);
    var fields = __spreadArray([
        "id"
    ], getListFields()
        .map(function (x) { return x.field.fetchField(); })
        .filter(function (x) { return x; }));
    var resource = useResourceCollection(__assign(__assign({ name: entity.name, dataSource: entity.dataSource }, restProps), { fields: fields, initialSorting: initialSorting || entity.initialSorting, initialFilters: initialFilters || entity.initialFilters }));
