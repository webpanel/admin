...
var EntityDetail = /** @class */ (function (_super) {
    ...
    EntityDetail.prototype.render = function () {
        ...
        return (React.createElement(Translation, null, function (t) { return (React.createElement(ResourceLayer, { name: entity.name, id: resourceID, dataSource: entity.dataSource, fields: [
                ...
                if (layout)
                    return layout;
                return (React.createElement(Card, { title: t(entity.title), loading: resource.loading && !resource.polling, extra: React.createElement(Link, { to: resourceID + "/edit" },
                        React.createElement(Button, { size: "small", htmlType: "button", icon: "edit" })) },
                    React.createElement(Form, null, entity.detailFields.map(function (field, i) { return (React.createElement(Form.Item, __assign({ key: field.name + "_" + i, label: t(field.title) }, formItemLayout), resource.data && field.render(resource.data))); }))));
            } })); }));
    };
    return EntityDetail;
}(React.Component));
