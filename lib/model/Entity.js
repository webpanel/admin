    ...
    function Entity(config) {
        ...
        this.structureItem = function () {
            return (React.createElement(Layout.StructureItem, { key: "/" + _this.structureName, name: _this.title, content: React.createElement(EntityList, { entity: _this, dataSource: _this.config.dataSource, detailButtonText: _this.config.showDetailPage ? 'Detail' : 'Edit' }) },
                ...
                React.createElement(Layout.StructureItem, { key: "/:id", name: "Detail", content: _this.getDetailPage }),
                React.createElement(Layout.StructureItem, { key: "/:id/edit", name: "Edit", content: _this.getEditPage })));
        };
        this.getDetailPage = function (route) {
            if (_this.config.showDetailPage) {
                return _this.detailLayout && typeof _this.detailLayout === 'function' ? (_this.detailLayout(_this, route)) : (React.createElement(EntityDetail, { entity: _this, route: route }));
            }
            return React.createElement(Redirect, { to: route.match.params.id + "/edit" });
        };
        this.getEditPage = function (route) {
            return _this.editLayout && typeof _this.editLayout === 'function' ? (_this.editLayout(_this, route)) : (React.createElement(EntityEdit, { entity: _this, route: route }));
        };
    }
