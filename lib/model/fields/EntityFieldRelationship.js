...
var EntityFieldRelationship = /** @class */ (function (_super) {
    ...
    function EntityFieldRelationship() {
        ...
        _this.updateFilterField = function (resource, operationName, value, customName) {
            var filterName = "" + (customName || _this.name) + (operationName ? '_' : '') + (operationName || '');
            var filters = resource.namedFilter(_this.name) || {};
            var filter = filters[_this.name] || {};
            filters[_this.name] = filter;
            if (value) {
        ...
        _this.valueForFilterField = function (resource, operationName, customName) {
            var filterName = "" + (customName || _this.name) + (operationName ? '_' : '') + (operationName || '');
            var filter = resource.namedFilter(_this.name);
            var fieldFilter = filter && filter[_this.name];
            if (!fieldFilter) {
                return undefined;
            }
            return fieldFilter[filterName];
        };
        _this.filterDropdownInput = function (mainResource) {
            ...
            return (React.createElement(ResourceCollectionLayer, { name: targetEntity.name, fields: [
                    'id'
                ].concat(_targetEntity.searchableFields.map(function (x) { return x.fetchField; })), dataSource: _targetEntity.dataSource, render: function (resource) {
                    var value = _this.valueForFilterField(mainResource, 'in', 'id');
                    return (React.createElement(React.Fragment, null,
                        React.createElement(ResourceSelect, { valueKey: "id", labelKey: function (value) {
                                return _targetEntity.render(value);
                            }, value: value, mode: _this.mode, allowClear: false, resourceCollection: resource, style: { minWidth: '200px' }, onChange: function (value) {
                                return _this.updateFilterField(mainResource, 'in', [value], 'id');
                            } }),
                        React.createElement(Button, { disabled: !_this.isFiltered(mainResource), onClick: function () { return _this.clearFilters(mainResource); }, icon: "delete" })));
                } }));
        };
        return _this;
    }
    ...
    EntityFieldRelationship.prototype.fieldElement = function (formContext, key, config) {
        ...
        var formItemLayout = config.formLayout === 'horizontal'
            ...
            : null;
        var addButtonMargin = config.formLayout === 'horizontal' ? '4px 0 0 4px' : '43px 0 0 4px';
        return (React.createElement(ResourceCollectionLayer, { key: key, name: _targetEntity.name, fields: [
                'id'
            ].concat(_targetEntity.searchableFields.map(function (x) { return x.name; })), initialSorting: _targetEntity.initialSorting, initialFilters: _targetEntity.initialFilters, dataSource: _targetEntity.dataSource, render: function (collection) { return (React.createElement("div", { style: {
                    display: 'flex',
                    justifyContent: 'space-between'
                } },
                React.createElement(FormField, __assign({ label: _this.title, name: _this.columnName, formContext: formContext, style: { width: '100%' } }, formItemLayout),
                    React.createElement(ResourceSelect, { valueKey: "id", labelKey: function (value) {
                            return _targetEntity.render(value);
                        }, mode: _this.mode, resourceCollection: collection })),
                React.createElement(Button, { size: "small", icon: "plus", style: {
                        ...
                        var infoWindow = Modal.info({
                            ...
                            okText: 'Close',
                            style: { minWidth: '60%' },
                            content: (React.createElement(EntityEdit, { onCreate: function (id) { return __awaiter(_this, void 0, void 0, function () {
                                    var updateValues;
    ...
    EntityFieldRelationship.prototype.isFiltered = function (resource) {
