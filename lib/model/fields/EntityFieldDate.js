...
var EntityFieldDate = /** @class */ (function (_super) {
    ...
    function EntityFieldDate() {
        ...
        _this.filterDropdownInput = function (resource) {
            return (_this.range ?
                (React.createElement(AntdDatePicker.RangePicker, { format: _this.config.format, onChange: function (value) {
                        _this.updateFilterField(resource, 'gte', moment(value[0]).startOf('day').toISOString());
                        _this.updateFilterField(resource, 'lte', moment(value[1]).endOf('day').toISOString());
                    } }))
                :
                    (React.createElement(DatePicker, { onChange: function (value) {
                            _this.updateFilterField(resource, 'gte', moment(value).startOf('day').toISOString());
                            _this.updateFilterField(resource, 'lte', moment(value).endOf('day').toISOString());
                        } })));
        };
        return _this;
    }
