...
var EntityFieldNumber = /** @class */ (function (_super) {
    ...
    function EntityFieldNumber(name, config, entity) {
        var _this = _super.call(this, name, config, entity) || this;
        _this.name = name;
        _this.config = config;
        _this.entity = entity;
        _this.filterDropdownInput = function (props) {
            var format = _this.config.format;
            var value = props.selectedKeys ? props.selectedKeys[0] : undefined;
            return (React.createElement(InputNumber, { style: { minWidth: "195px" }, key: "number_field_" + _this.entity.name + "_" + _this.valuePropName, placeholder: "Number", value: value, onChange: function (value) {
                    return props.setSelectedKeys(value ? [value] : []);
                }, formatter: function (value) { return numeral(value).format(format); } }));
        };
        if (typeof config.format !== "undefined" &&
            typeof config.render === "undefined") {
            _this.config.render = function (values) {
                return numeral(values[name]).format(config.format);
            };
        }
        return _this;
    }
    EntityFieldNumber.prototype.inputElement = function (props) {
        ...
        var onChangeProp = onChange
            ...
            : undefined;
        return (React.createElement(InputNumber, __assign({ style: { minWidth: "195px" }, key: "number_field_" + this.entity.name + "_" + this.valuePropName, formatter: function (value) { return numeral(value).format(format); } }, props, { onChange: onChangeProp })));
    };
